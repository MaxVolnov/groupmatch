...
# ── уборка ───────────────────────────────────────────────────────────────────
#
# Скрипт создаёт в проде живой аккаунт, группу и встречи. Раньше он оставлял их
# после каждого прогона, и админка постепенно наполнялась мусором.
#
# Через trap EXIT, а не строкой в конце: при set -e любая упавшая проверка
# завершает скрипт немедленно, и «уборка в конце» не выполнялась бы именно
# тогда, когда мусора остаётся больше всего.
#
# Группу отдельно удалять не нужно: смоук-юзер в ней единственный участник, и
# она уходит вместе с ним — вместе со слотами, встречами и приглашениями.
cleanup() {
  local exit_code=$?

  if [ -z "${ACCESS_TOKEN:-}" ]; then
    echo ""
    echo "⚠️  Уборка пропущена: аккаунт создан не был или вход не удался."
    echo "   Если пользователь ${EMAIL} всё же появился, удалите его в админке."
    exit $exit_code
  fi
  ...
  # `|| status="000"` обязателен: при set -e сетевой отказ curl (недоступный
  # хост, таймаут) убивал бы ловушку прямо здесь, и скрипт завершался бы
  # ненулевым кодом, но не сказав ни слова о том, что мусор остался.
  local status
  status=$(curl -s -o /dev/null -w "%{http_code}" --max-time 30 -X DELETE "${BASE_URL}/api/v1/me" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${ACCESS_TOKEN}" \
    -d "{\"password\":\"${PASSWORD}\"}") || status="000"

  if [ "$status" = "204" ]; then
    echo "✅ тестовый аккаунт удалён → HTTP 204"

    # Проверяем, что группа ушла вместе с ним: это и есть каскад из пункта 2.
    if [ -n "${GROUP_ID:-}" ]; then
      local group_status
      group_status=$(curl -s -o /dev/null -w "%{http_code}" --max-time 30 \
        "${BASE_URL}/api/v1/groups/${GROUP_ID}" \
        -H "Authorization: Bearer ${ACCESS_TOKEN}") || group_status="000"
      # Токен уже отозван, поэтому здесь ожидается 401, а не 404: важно лишь,
      # что группа не доступна. Само её удаление проверяется тестом
      # EmptyGroupCleanupTest, здесь достаточно того, что аккаунта нет.
      echo "ℹ️  группа ${GROUP_ID} уходит каскадом вместе с последним участником"
      echo "   (запрос к ней после отзыва токена → HTTP ${group_status})"
    fi
  else
    # Явно и с ненулевым кодом: молча завершиться нулём, оставив мусор в
    # проде, — худший из возможных исходов.
    echo "❌ НЕ УДАЛОСЬ удалить тестовый аккаунт → HTTP ${status} (000 = до API не достучались)"
    echo "   Пользователь ${EMAIL} остался в базе, удалите его в админке."
    echo "   Фильтр «Тестовые» в списке пользователей показывает такие аккаунты."
    if [ "$exit_code" -eq 0 ]; then exit_code=1; fi
  fi

  # exit, а не return: в bash 5 return из EXIT-ловушки код возврата задаёт, а в
  # bash 3.2 (он до сих пор стоит в macOS по умолчанию) — нет, и провалившаяся
  # уборка завершилась бы нулём. Ровно того, что запрещено.
  exit $exit_code
}
trap 'cleanup' EXIT
...
echo "   group id:  ${GROUP_ID}"
# Уборка выполнится после этой строки — она висит на trap EXIT.
