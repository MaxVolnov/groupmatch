...
### Added
- **Видно, кто именно свободен в каждой получасовке** — под таблицей появляется список имён для ячейки, на которую навели или по которой коснулись. Раньше по таблице читалось только «сколько человек», и назначать встречи по одной с каждым приходилось, сверяясь со вкладкой участников. На телефоне для этого не нужен новый жест: список наполняется тем же касанием, которым выделяют время или назначают встречу, и сам подтягивается к экрану. Имена показываются, если у группы включено «показывать участников»

### Changed
- Версия проекта приведена к 0.15.0 во всех местах, где она записана: она стояла на 0.10.0 пять релизов подряд

### Fixed
- **Настройка «показывать участников» действует сразу** — раньше включённая галочка не меняла таблицу до часа: кэш теплокарты сбрасывался только правкой чьего-нибудь расписания. Со стороны владельца это выглядело как настройка, которая не работает

---

## [Unreleased]

⚠️ Этот раздел описывает работу, которая **уже вышла** — в 0.11.0 и 0.12.0.
Секций для 0.11.0–0.14.0 в файле нет: релизы тегировались без них, и часть
работы (0.12.0–0.14.0) сюда вообще не попала. Разносить накопленное по
релизам задним числом — это угадывать, какая строка к какому тегу
относится, поэтому раздел оставлен как есть. Новое пишется выше, в 0.15.0.
