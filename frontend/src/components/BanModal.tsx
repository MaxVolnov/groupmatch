...
export function BanModal({ user, onClose }: Props) {
  ...
  return (
    <Modal
      title={`${t('admin.banModalTitle')} ${user?.displayName ?? ''}`}
      open={user !== null}
      onClose={handleClose}
      footer={
        <>
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('admin.reason')}</label>
          <textarea
            value={reason}
        ...
        {ban.error && (
          <p className="text-sm text-red-600 dark:text-red-400">{t('admin.banFailed')}</p>
        )}
      </div>
    </Modal>
  )
