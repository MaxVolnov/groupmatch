 ...
 *
 * Своего таймаута не было вовсе: запрос висел около тридцати секунд — это
 * умолчание где-то в цепочке, не наше, — и всё это время интерфейс не подавал
 * признаков жизни. Из-за фильтрации трафика соединение с API периодически не
 * устанавливается, и такие тридцать секунд человек видит регулярно.
 *
 * Десять секунд взяты с запасом к измеренному. На проде вход с проверкой
 * Argon2id занимает 0,7 с, самая тяжёлая операция — создание серии на
 * максимальные 200 слотов — 0,18 с на стенде. Запас больше чем десятикратный:
 * ложное срабатывание означало бы новое враньё, только другое.
 */
export const REQUEST_TIMEOUT_MS = 10_000

/**
 * Таймаут для операций, где ложное срабатывание дороже ожидания.
 *
 * Дело не в скорости — по замерам они укладываются в доли секунды. Дело в
 * последствиях: создание серии не идемпотентно, и человек, увидевший «нет
 * связи» на запросе, который на самом деле дошёл, нажмёт ещё раз и получит
 * двести дублирующихся слотов. Лучше подождать втрое дольше, чем разбирать
 * такое.
 */
export const LONG_OPERATION_TIMEOUT_MS = 30_000

/** Конфиг для таких операций — чтобы значение не разъехалось по вызовам. */
export const longOperation = () => ({ timeout: LONG_OPERATION_TIMEOUT_MS })
