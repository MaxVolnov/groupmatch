...
> Этот раздел писался до того, как бэкенд поднялся на Timeweb, и описывает
> порядок в общих чертах. Пошаговый чек-лист с точками отката, командами
> финального переноса и разбором отказов `.ics`/CORS —
> **`docs/timeweb-cutover.md`**. При расхождении верен он.
