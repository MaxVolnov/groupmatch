...
- QR-код: генерация через `qrcode` + коррекция ошибок H, UTM-метка (`utm_source=qr&utm_medium=print&utm_campaign=alpha`), логотип-плейсхолдер 22% площади, прошёл проверку реальным декодером
