...
> Backend: Spring Boot 4 + Java 25 + PostgreSQL 18 + Valkey → Timeweb Cloud, Москва (`api.groupmatch.app`)
> Auth: Custom JWT (access 15 min / refresh 14 days regular / 90 days guest) + Argon2id
>
> Действующее описание прода — `docs/prod-runbook.md`. Текущий бэклог —
> `docs/backlog.md`. Этот файл — летопись фаз, а не состояние на сегодня.

---

## 🎨 Брендинг — постоянные правила

> Не changelog: это действующие соглашения, а не запись о прошедшей работе.

### Написание названия сервиса

Название пишется по-разному в тексте и в графике. **Расхождение осознанное,
это не баг.** Приводить одно к другому не нужно.

| Где | Как | Примеры |
|---|---|---|
| **В текстах** — торговое название | `GroupMatch`, заглавная **M** | интерфейс, `<title>`, `site.webmanifest`, тексты писем, оферта и юридические документы, README, патчноуты, посты в Telegram-канале |
| **В логотипе и на OG-превью** — графическое начертание, согласовано с дизайнером | `Groupmatch`, строчная **m** | `og-image.jpg`, логотипы, фавиконы |

Что делать при встрече:

- **`Groupmatch` в тексте кода или контента** — ошибка, исправлять на `GroupMatch`.
- **`GroupMatch` в графическом ассете** — не трогать. Ассеты правит только
  дизайнер; переверстать логотип «чтобы совпадало» нельзя.

### Палитра

Единственный источник значений — `frontend/src/shared/brand.css`; оба CSS-входа
(`site.css`, `promo.css`) его импортируют, дублировать значения не нужно.
Те же цвета продублированы токенами `gm.*` в `frontend/tailwind.config.cjs` —
это отдельный слой для SPA, который свёрстан на Tailwind-утилитах; при
изменении палитры править оба места.

| Токен | Значение | Назначение |
|---|---|---|
| `--gm-bg` | `#040929` | основной фон, `theme-color` |
| `--gm-accent` | `#055EC2` | основной синий, **только заливка** (на фоне даёт 3.15:1) |
| `--gm-accent-light` | `#5DB7F4` | акцентный текст, ссылки, иконки (6.51:1) |
| `--gm-accent-deep` | `#143291` | дополнительный глубокий синий |
| `--gm-text` | `#FFFFFF` | текст |

### Ассеты

- Иконки (`favicon.ico`, `favicon-96x96.png`, `apple-touch-icon.png`,
  `web-app-manifest-*`) — вариант знака на тёмной подложке. Браузеры и ОС не
  переключают иконку по теме, а фон вкладки и домашнего экрана мы не
  контролируем.
- Внутри интерфейса, где тема известна, — прозрачные варианты
  `logo-on-dark.svg` и `logo-on-light.svg`. Имя указывает на целевой фон:
  у `-on-dark` буква G белая и на светлой шапке исчезнет.
- OG-превью — JPEG, а не PNG: картинка представляет собой градиент во весь
  кадр, PNG такое жмёт вчетверо хуже при худшей же точности.
- Имена файлов только ASCII: в архивах от дизайнера размеры записаны
  кириллической «х» (U+0445), которая ломается на CI и в путях сборки.
...
- Брендовая палитра — Концепт 2 (синий), значения сняты с исходного макета дизайнера: `#040929` / `#055EC2` / `#6499D0`. *(Историческое состояние на момент фазы 9.0. Финальные значения — в разделе «Брендинг» в начале документа; светло-голубой с тех пор изменился на `#5DB7F4`.)*
...
- ✅ **Гостевые аккаунты удаляются раньше обещанного срока** — закрыто в v0.9.0: TTL считается от последней активности, 90 дней согласованы с refresh-токеном и текстом на `/legal`. Исходная формулировка: `GuestCleanupJob` чистит по `created_at` (30 дней), UI обещает 90 дней (совпадает с refresh TTL). Активный гость теряет все данные на 31-й день. Исправить: считать от последней активности, согласовать с 90-дневным TTL (текст на `/legal` сегодня приведён в соответствие с ТЕКУЩИМ поведением джобы — «30 дней с момента создания» — это не решает сам P0, лишь убирает ложное обещание «90 дней» из юридического документа; после фикса этого пункта текст `/legal` нужно обновить снова)
- ⏳ **Аудит разлогинов для обычных (не гостевых) пользователей** — разобрано, но не починено: причина в том, что сессии живут только в кэше без подтверждённой персистентности (`docs/archive/session-audit-2026-08.md`, резолюция; `docs/backlog.md`, пункт 6). Исходная формулировка: 15-минутный P0 из Фазы 8 закрыт и подтверждён 4-часовой сессией, но отдельно поступила жалоба на разлогин «примерно раз в 2 недели» у обычных юзеров. Это не тот же баг — вероятно, TTL refresh-токена (14 дней) или его ротация. Нужно проверить отдельно, не считать закрытым автоматически вместе с гостевым багом
- ✅ **Webhook ЮKassa без проверки подписи** — закрыто в v0.9.0: HMAC-SHA256 + IP-allowlist, fail-closed. Исходная формулировка: `YooKassaService.handleWebhook()` публичный, принимает любой POST и активирует подписку/повышает план без валидации источника. Добавить проверку подписи + IP-диапазонов ЮKassa
- ✅ **Обход rate limit через X-Forwarded-For** — закрыто в v0.9.0: `ClientIpResolver` берёт самый правый недоверенный хоп и верит заголовку только от пиров из `TRUSTED_PROXIES`. Исходная формулировка: `RateLimitFilter.resolveClientIp()` доверяет заголовку без проверки доверенного прокси; позволяет обойти лимит на `/signin` (брутфорс паролей). Брать IP из `RemoteAddr` за настроенным trusted-proxy

### 9.2 Важные P1 (закрыть до или сразу после запуска беты)

- ✅ **Подделка `X-Forwarded-For` через доверенные диапазоны Cloudflare** — риска в описанном виде больше нет, снят переездом. ⚠️ Пункт был написан в настоящем времени как принятый риск и в таком виде вводил в заблуждение: тот, кто искал, что известно про подделку XFF, находил утверждение, что диапазоны Cloudflare доверены, — и либо записывал несуществующую дыру в задачи, либо «чинил» `TRUSTED_PROXIES`, то есть настройку, от которой зависит счёт лимита на вход.
  **Что изменилось.** Railway отключён (его удаление — открытый пункт в `docs/backlog.md`, раздел «На владельце»). `TRUSTED_PROXIES` диапазонов Cloudflare **не содержит намеренно** — действующее описание в `backend/src/main/resources/application.yml`, там же объяснено, почему их туда нельзя добавлять просто так. Доверие к диапазонам Cloudflare с сентября 2026 включается отдельным флагом `TRUST_CLOUDFLARE_PROXY`, по умолчанию выключенным.
  **Что осталось.** Прямой адрес приложения на площадке (`maxvolnov-groupmatch-4a45.twc1.net`) по-прежнему отвечает мимо `api.groupmatch.app`. Пока флаг выключен, подделки это не даёт; в день включения флага — даст. Поэтому закрытие прямого доступа (Authenticated Origin Pulls либо ограничение по адресам) стоит в очереди бэклога **перед** включением флага, а не после.
  **Исходная формулировка (для истории):** «риск принят, не чинить сейчас; `groupmatch-production.up.railway.app` остаётся публично доступным мимо `api.groupmatch.app`, а `TRUSTED_PROXIES` с переездом на Cloudflare теперь доверяет всем его диапазонам (см. `docs/archive/api-subdomain-migration.md`)».
...
- ✅ **`GlobalExceptionHandler.handleGenericException()` не логирует** — закрыто в сентябре 2026: необработанные исключения пишутся через `log.error` со стектрейсом, отказы валидации — через `log.info` с перечнем полей. Осталось отдельным пунктом бэклога другое: 404, 405 и битое тело запроса по-прежнему отдают 500, и внешний мониторинг из-за этого не отличает клиентскую ошибку от серверной (аудит 2)
