...
Первые семь диапазонов — прежнее значение (loopback и приватные сети, ими
приходит внутренний пир Railway), остальные пятнадцать — IPv4 Cloudflare.
Убирать приватные диапазоны нельзя: без них перестанет доверяться сам пир, и
`X-Forwarded-For` будет игнорироваться целиком.

Актуальность списка проверять по `curl -s https://www.cloudflare.com/ips-v4` —
Cloudflare его изредка меняет, а в `application.yml` он намеренно не зашит,
чтобы обновлять переменной без релиза.
...
**IPv6-диапазоны Cloudflare в список не вошли.** Если Cloudflare пойдёт к
origin по IPv6 (а он так делает, когда у origin есть AAAA), хопом в цепочке
окажется IPv6-адрес Cloudflare — его в списке нет, и вернётся именно он, то
есть ровно та проблема, которую мы чиним. Взять список и дописать:

```bash
curl -s https://www.cloudflare.com/ips-v6
```

По памяти не привожу. Проверить, какой именно адрес приходит, поможет тот же
DEBUG-шаг: если в логе виден `2a06:...` или другой IPv6 — список неполный.

**Доверие всем диапазонам Cloudflare ослабляет P0-фикс.** Наш origin публично
доступен по прямому адресу Railway, и атакующий может завести собственную зону
в Cloudflare, направить её на него и слать запросы с подделанным
`X-Forwarded-For`: хоп будет из доверенных диапазонов, и подделка сработает.
Обход лимита на `/auth/signin` — то, ради чего P0-фикс и делался.

Это неизбежная плата за схему «Cloudflare перед origin», лечится закрытием
origin от всех, кроме своего Cloudflare:

- Cloudflare → SSL/TLS → Origin Server → **Authenticated Origin Pulls**, либо
- секретный заголовок, который проставляет Transform Rule в Cloudflare, а
  приложение проверяет на входе, либо
- ограничение доступа к Railway по IP-диапазонам Cloudflare (если Railway это
  умеет на текущем тарифе).

До беты приемлемо оставить как есть: атака требует настроенной зоны и знания
прямого адреса origin, а выигрыш — обход лимита на перебор паролей, который
дополнительно упирается в Argon2. Но в бэклог занести стоит.
